...
@TeleOp (name = "!OneController", group = "OldRobot")
...
public class  teleOPoneController extends LinearOpMode{
    ...
    Servo rightAdjust;
    Servo leftAdjust;
    double coverClosed = 0;
    double leftAdjustOpen = 0;
    double rightAdjustOpen = 0;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while(opModeInInit() && !isStopRequested()){
            plate.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
            coverServo.setPosition(coverClosed);
        }

        while (opModeIsActive()){
            rightAdjust.setPosition
                    (rightAdjustOpen);
            leftAdjust.setPosition
                    (leftAdjustOpen);
            ...
            switch(state){
                case EMPTY:
                    ...
                    if(!forcedEmptyOnce && !full && triggerOnce) intake.setPower(1);
            ...
            if(gamepad1.right_bumper && ballsCollected > -1){
            ...
            if(gamepad1.left_bumper && ballsCollected > -1){
                if(!leftBumperToggle)
                {
                    forcedEmptyOnce = true;
                    state = Intake_Sensor.OUTAKE;
...
//            } else dpadLeftToggle = false;
            ...
            if(gamepad1.right_trigger < 0.01){
                if(!triggerToggle){
                    if(triggerOnce) intake.setPower(1);
